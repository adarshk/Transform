...
private:
    ...
    gl::BatchRef		mPrimitive_for_plane;
...
void GeometryApp::draw()
{
    ...
    mPhongShader->uniform("mixAmount", mxAm);
    mPhongShader->uniform("worldUp", mCamera.getWorldUp());
    ...
    if (mxAm > 0.05) {
        mPhongShader->uniform("flag", false);
    }
    else{
        mPhongShader->uniform("flag", true);
    }
	...
	if( mPrimitive ) {
		...
		gl::pushModelView();
        gl::translate(vec3(0,-mxAm*100,-mxAm*100));
...
void GeometryApp::createPrimitive(void)
{
    ...
    mPrimitive_for_plane = gl::Batch::create( mesh, mPhongShader );
	...
	try {
		mPhongShader = gl::GlslProg::create( gl::GlslProg::Format()
                                            .vertex(
                                                    ...
                                                    "vec3 goalPosition = 10.0 * vec3(-ciTexCoord0.x,ciTexCoord0.y,-ciTexCoord0.x) ;\n"
                                                    //
                                                    
                                                    
                                                    /*Oval Shaped
                                                     "vec3 goalPosition = 10.0 * vec3(0,0,-ciTexCoord0.y) ;\n"
                                                     "vec4 newPosition = mix(ciPosition,vec4(goalPosition,1.0),0.8);\n"
                                                     */
